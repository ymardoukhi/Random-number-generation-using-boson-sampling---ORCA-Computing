...
def von_neumann_sim(n, m, num_params, v, seed):
    """
    function that applies the von Neumann protocol on two 
    samples of a Boson Sampler and return the binary string
    ...
    return:
        von_neumann_str (str): the binary string generated by 
    """
    ...
    # get two samples from the boson sampler
    shots = []
    for _ in range(2):
        shots.append(eng.run(program=arch.prog, args=args_dict).samples[0])

    # initialise the VonNeumann protocol and get the binary
    # string out of the two samples
